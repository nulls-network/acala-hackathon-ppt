...
### Stablecoins Analytics (24H)
Major stablecoin transactions on mainstream blockchain networks

| Network | Token type | Transfers Count(24h) |  Transfer Amount（24h） |
| :----------- | :------------: |:------------: | ------------: |
...
* :WEB3\::{.text-label} Easy usage for users and merchants with just one wallet.
* :Abstracty\::{.text-label}  Easy accessible SDKs for developers and user-friendly front-end for users without knowing the intricate underlying layers.
* :Trustless\::{.text-label} Smart contracts and other trusted technologies to ensure the functioning with minimum trust.
* :Safety\::{.text-label} Strong economic incentives for nodes to operate reliably and accurately.
* :Reliable\::{.text-label} SubStrate-based Layer2 for reliable performance[(_***Performance Report Link***_)](https://github.com/nulls-network/chain-loadtest).
* :Indexed\::{.text-label} Indexing service built by the founding team for developers and merchants to build and use.
* :Developer\::{.text-label} Improving products and expanding product boundaries by injecting incentives into the community.
{.description}

<slide :class="size-60" class="bg-black">
### Integration with Acala 

DPayment accepts payments from multiple blockchain networks. Once the payment is received in the non-clearing chain, equivalent assets on the clearing chain will be minted for accounting using a cross-chain mechanism.

:::shadowbox
## Powering the Acala Network.
 DPayment will use the Acala network for bookkeeping and registering the cash flow, and merchants will use the Acala network for asset management. DPayment's Callback mode can also offer a variety of usage scenarios to the Acala network.
（[IDO Demo](https://opendao.dev)）。

---

## StableCoin - aUSD
After receiving aUSD, merchants can withdraw StableCoins of equivalent value through withdrawal and payment operations, such as USDT/USDC/BUSD/DAI in other networks.
:::
...
sequenceDiagram
    ...
    Buyer ->> PaymentContract : Select network and payment currency to pay for order voucher
    PaymentContract -->> Merchan: Notify merchants of payment status
    Note left of PaymentContract : The merchant can also <br /> determinethe transaction status by <br />monitoring the contract events <br />of the clearing network <br />
    Merchant ->> Buyer : Delivering goods to buyer
    loop monitoring transactions
        Clearing -->> PaymentContract : Synchronize order registration and billing
    end 
    Note left of Clearing : Through OffChain-Worker<br/>Monitor orders in other networks<br/>payment message，and recorded in<br/>in its own clearing network.
```
<slide :class="size-80 bg-white" class="bg-black" >
## Changes in the way you use Web3

```mermaid
sequenceDiagram
Note right of Merchant1 : Merchants need to <br />provide identification information and communication keys
Merchant1 ->> PayPal : Apply for an account
Merchant2 ->> DPayment : Sign with wallet's private key
Note left of DPayment : Doesn't need to Bidding the account with the private key <br /> like rraditional institutions <br />
Note left of DPayment : The payment institution sends its own communication key<br /> to the merchant, and the merchant starts the technical integration
PayPal ->> Merchant1 : Approved, successful account opening
DPayment ->> Merchant2 : Obtain the signaturethrough Recover and directly bookkeeping
Merchant1 ->> PayPal : Sign order request with private key
PayPal ->> Merchant1 : Verify signature with private key and register order
```
...
When **Visa** and **Master** announced deny of service for ordinary Russian users, **DPayment** 's opportunity is coming! {.text-into.animated.delay-800.fadeIn}
